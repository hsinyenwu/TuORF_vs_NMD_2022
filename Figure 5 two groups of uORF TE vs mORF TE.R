...
RiboD1 <- read.delim("~/Desktop/CTRL_v1/data/Ribo/D1/D1.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RiboD2 <- read.delim("~/Desktop/CTRL_v1/data/Ribo/D2/D2.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RiboD3 <- read.delim("~/Desktop/CTRL_v1/data/Ribo/D3/D3.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RiboD5 <- read.delim("~/Desktop/CTRL_v1/data/Ribo/D5/D5.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RiboD6 <- read.delim("~/Desktop/CTRL_v1/data/Ribo/D6/D6.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RiboD7 <- read.delim("~/Desktop/CTRL_v1/data/Ribo/D7/D7.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")

Ribo <- data.frame(gene_id=RiboD1$gene_id,D1TPM=RiboD1$TPM,D2TPM=RiboD2$TPM,D3TPM=RiboD3$TPM,D5TPM=RiboD5$TPM,D6TPM=RiboD6$TPM,D7TPM=RiboD7$TPM)
Ribo$Ribo_Mean <- rowMeans(Ribo[,2:7])

RNAD1 <- read.delim("~/Desktop/CTRL_v1/data/RNA/D1/D1.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RNAD2 <- read.delim("~/Desktop/CTRL_v1/data/RNA/D2/D2.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RNAD3 <- read.delim("~/Desktop/CTRL_v1/data/RNA/D3/D3.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RNAD5 <- read.delim("~/Desktop/CTRL_v1/data/RNA/D5/D5.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RNAD6 <- read.delim("~/Desktop/CTRL_v1/data/RNA/D6/D6.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")
RNAD7 <- read.delim("~/Desktop/CTRL_v1/data/RNA/D7/D7.genes.results",header=T,sep="\t",stringsAsFactors = F,quote = "")

RNA <- data.frame(gene_id=RNAD1$gene_id,D1TPM=RNAD1$TPM,D2TPM=RNAD2$TPM,D3TPM=RNAD3$TPM,D5TPM=RNAD5$TPM,D6TPM=RNAD6$TPM,D7TPM=RNAD7$TPM)
RNA$RNA_Mean <- rowMeans(RNA[,2:7])


TE <- inner_join(RNA,Ribo,by="gene_id")

TE$TE <- TE$RNA_Mean/TE$Ribo_Mean
...
upf1upf3_Col_genelevel_ratio$Category <- ifelse(upf1upf3_Col_genelevel_ratio$gene_id %in% NMD$gene_id,"NMD",ifelse(upf1upf3_Col_genelevel_ratio$gene_id %in% ORF_max_filt_uORF_low$gene_id, "Low", ifelse(upf1upf3_Col_genelevel_ratio$gene_id %in% ORF_max_filt_uORF_high$gene_id,"High","Others")))
upf1upf3_Col_genelevel_ratio$Category <- factor(upf1upf3_Col_genelevel_ratio$Category, levels=c("NMD", "Low", "High", "Others"), labels=c("NMD", "Low", "High", "Others"))
...
upf1pad4_pad4_genelevel_ratio$Category <- ifelse(upf1pad4_pad4_genelevel_ratio$gene_id %in% NMD$gene_id,"NMD",ifelse(upf1pad4_pad4_genelevel_ratio$gene_id %in% ORF_max_filt_uORF_low$gene_id, "Low", ifelse(upf1pad4_pad4_genelevel_ratio$gene_id %in% ORF_max_filt_uORF_high$gene_id,"High","Others")))
upf1pad4_pad4_genelevel_ratio$Category <- factor(upf1pad4_pad4_genelevel_ratio$Category, levels=c("NMD", "Low", "High", "Others"), labels=c("NMD", "Low", "High", "Others"))
...
smg7pad4_pad4_genelevel_ratio%>% group_by(Category) %>% dplyr::summarize(Median = median(ratio, na.rm=TRUE)/0.938)
...
# ggsave("~/Desktop/atRTD3/TuORF_TE_2_groups_vs_NMDt_vs_Other_in_NMD_mutants.pdf",
#        plot = p4,
#        units = "in",
#        width = 14.5,
#        height = 5)
