#############################################################################################
# Shoot+Root Hsu 2016: Ratio of RNA levels in NMD mutants vs Control
#############################################################################################
rm(list=ls())
library(openxlsx)
library(dplyr)
...
#RiboTaper output
ORF_max_filt <- read.xlsx("~/Desktop/Hsu2016_Root_Shoot_TuORF.xlsx",sheet="Combined")
table(ORF_max_filt$category)
# dORF     ncORFS  ORFs_ccds Overl_dORF       uORF
# 208        379      37361         15       2093
ORF_max_filt_uORF <- ORF_max_filt %>% filter(category=="uORF") %>% filter(!(gene_id %in% NMD$gene_id))
head(ORF_max_filt_uORF,2)
nrow(ORF_max_filt_uORF) #138
...
ORF_max_filt_uORF_low <- dplyr::top_n(ORF_max_filt_uORF_mORF,-69,uORF_mORF_Ribo_ratio)
ORF_max_filt_uORF_high <- dplyr::top_n(ORF_max_filt_uORF_mORF,69,uORF_mORF_Ribo_ratio)
...
# ORF_max_filt_uORF_mORF <- inner_join(ORF_max_filt_uORF,ORF_max_filt_mORF) %>% mutate(uORF_mORF_Ribo_normcount=(ORF_P_sites/ORF_length))
# dim(ORF_max_filt_uORF_mORF) #[1] 1438   42
# ORF_max_filt_uORF_low <- dplyr::top_n(ORF_max_filt_uORF_mORF,-300,uORF_mORF_Ribo_normcount)
# ORF_max_filt_uORF_high <- dplyr::top_n(ORF_max_filt_uORF_mORF,300,uORF_mORF_Ribo_normcount)
# ORF_max_filt_uORF_mid <- ORF_max_filt_uORF %>% filter(!(gene_id %in% c(ORF_max_filt_uORF_low$gene_id,ORF_max_filt_uORF_high$gene_id)))

# TE_CDS$Category <- ifelse(TE_CDS$gene_id %in% ORF_max_filt_uORF_low$gene_id,"Low",ifelse(TE_CDS$gene_id %in% ORF_max_filt_uORF_mid$gene_id, "Mid", ifelse(TE_CDS$gene_id %in% ORF_max_filt_uORF_high$gene_id,"High","Others")))
# TE_CDS$Category <- factor(TE_CDS$Category, levels=c("Low", "Mid", "High", "Others"), labels=c("Low", "Mid", "High", "Others"))

TE_CDS %>% group_by(Category) %>% summarise_at(vars(-gene_id),list(mean = mean, median = median))

# A tibble: 4 × 7
# Category RNA_mean Ribo_mean TE_mean RNA_median Ribo_median
# <fct>       <dbl>     <dbl>   <dbl>      <dbl>       <dbl>
# 1 NMD          17.5     21.6    0.937       4.7         3.08
# 2 Low          48.9     65.0    1.14       14.9        15.0
# 3 High         14.8      9.08   0.634       9.7         5.63
# 4 Others       32.0     44.4    1.38        7.84        7.88
...
Col_1_genelevel <- Col_1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
Col_2_genelevel <- Col_2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf1upf3_1_genelevel <- upf1upf3_1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf1upf3_2_genelevel <- upf1upf3_2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
...
upf1upf3_Col_genelevel_ratio$Category <- ifelse(upf1upf3_Col_genelevel_ratio$gene_id %in% NMD$gene_id,"NMD",ifelse(upf1upf3_Col_genelevel_ratio$gene_id %in% ORF_max_filt_uORF$gene_id, "uORF","Others"))
upf1upf3_Col_genelevel_ratio$Category <- factor(upf1upf3_Col_genelevel_ratio$Category, levels=c("NMD", "uORF", "Others"), labels=c("NMD", "uORF", "Others"))
...
vsteps <- yvalue(ystart=12.35,stepsize=0.5,num=3)
...
ColRNA1_genelevel <- ColRNA1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
ColRNA2_genelevel <- ColRNA2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf2RNA1_genelevel <- upf2RNA1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf2RNA2_genelevel <- upf2RNA2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
...
upf2_Col_genelevel_ratio$Category <- ifelse(upf2_Col_genelevel_ratio$gene_id %in% NMD$gene_id,"NMD",ifelse(upf2_Col_genelevel_ratio$gene_id %in% ORF_max_filt_uORF$gene_id, "uORF", "Others"))
upf2_Col_genelevel_ratio$Category <- factor(upf2_Col_genelevel_ratio$Category, levels=c("NMD", "uORF", "Others"), labels=c("NMD", "uORF", "Others"))
...
upf2_Col_genelevel_ratio %>% group_by(Category) %>% dplyr::summarize(Median = median(ratio, na.rm=TRUE)/0.998)
# A tibble: 4 × 2
# Category Median
# <fct>     <dbl>
#   1 NMD       1.72
...
vsteps <- yvalue(ystart=4.5,stepsize=0.26,num=3)
...
pad4_1_genelevel <- pad4_1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
pad4_2_genelevel <- pad4_2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
pad4_3_genelevel <- pad4_3 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf1pad4_1_genelevel <- upf1pad4_1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf1pad4_2_genelevel <- upf1pad4_2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
upf1pad4_3_genelevel <- upf1pad4_3 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
...
upf1pad4_pad4_genelevel_ratio$Category <- ifelse(upf1pad4_pad4_genelevel_ratio$gene_id %in% NMD$gene_id,"NMD",ifelse(upf1pad4_pad4_genelevel_ratio$gene_id %in% ORF_max_filt_uORF$gene_id, "uORF","Others"))
upf1pad4_pad4_genelevel_ratio$Category <- factor(upf1pad4_pad4_genelevel_ratio$Category, levels=c("NMD", "uORF", "Others"), labels=c("NMD", "uORF", "Others"))
...
upf1pad4_pad4_genelevel_ratio%>% group_by(Category) %>% dplyr::summarize(Median = median(ratio, na.rm=TRUE)/0.877)
...
vsteps <- yvalue(ystart=13,stepsize=0.6,num=3)
...
pad4_2_genelevel <- pad4_2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
pad4_3_genelevel <- pad4_3 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
smg7pad4_1_genelevel <- smg7pad4_1 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
smg7pad4_2_genelevel <- smg7pad4_2 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
smg7pad4_3_genelevel <- smg7pad4_3 %>% group_by(gene_id) %>% dplyr::summarise(gene_tpm = sum(tpm)) %>% as.data.frame()
...
smg7pad4_pad4_genelevel_ratio$Category <- ifelse(smg7pad4_pad4_genelevel_ratio$gene_id %in% NMD$gene_id,"NMD",ifelse(smg7pad4_pad4_genelevel_ratio$gene_id %in% ORF_max_filt_uORF$gene_id, "uORF","Others"))
smg7pad4_pad4_genelevel_ratio$Category <- factor(smg7pad4_pad4_genelevel_ratio$Category, levels=c("NMD", "uORF", "Others"), labels=c("NMD", "uORF", "Others"))
...
smg7pad4_pad4_genelevel_ratio%>% group_by(Category) %>% dplyr::summarize(Median = median(ratio, na.rm=TRUE)/0.944)
...
vsteps <- yvalue(ystart=5.2,stepsize=0.2,num=3)
...
p4 <- grid.arrange(pupf1upf3_Col_RNA_ratio + theme(legend.position="none"),
                   ...
                   top=textGrob("   Figure. Hsu2016 Shoot+Root TuORF genes vs mRNA ratio between NMD mutants and WT", gp=gpar(fontsize=13), x = 0, hjust = 0))

############################
ORF_max_filt_A <- read.delim(file="~/Desktop/uORFs_miRNA/ORFs_max_filt_Araport_both_round_reads",header=T,stringsAsFactors=F,sep="\t")
table(ORF_max_filt_A$category)
# dORF     ncORFS  ORFs_ccds Overl_dORF       uORF
# 208        379      37361         15       2093
ORF_max_filt_A_uORF <- ORF_max_filt_A %>% filter(category=="uORF")

ORF_max_filt_uORF_RS <- read.xlsx("~/Desktop/Hsu2016_Root_Shoot_TuORF.xlsx",sheet="Combined")
ORF_max_filt_uORF_S <- read.xlsx("~/Desktop/Hsu2016_Root_Shoot_TuORF.xlsx",sheet="Shoot")
ORF_max_filt_uORF_R <- read.xlsx("~/Desktop/Hsu2016_Root_Shoot_TuORF.xlsx",sheet="Root")
length(intersect(ORF_max_filt_A_uORF$gene_id,ORF_max_filt_uORF_RS$gene_id)) #151

length(unique(ORF_max_filt_uORF_RS$gene_id)) #178 genes
table(ORF_max_filt_uORF_RS$gene_id)
length(unique(ORF_max_filt_uORF_RS$ORF_id_tr)) #201

length(intersect(ORF_max_filt_uORF_RS$gene_id,ORF_max_filt_A_uORF$gene_id)) #151
length(intersect(ORF_max_filt_uORF_RS$gene_id,ORF_max_filt_A_uORF$gene_id)) #151


#After removing NMD targets
ORF_max_filt_uORF_RS2 <- ORF_max_filt_uORF_RS %>% filter(!(gene_id %in% NMD$gene_id))
length(unique(ORF_max_filt_uORF_RS2$gene_id)) #166 genes

ORF_max_filt_A_uORF2 <- ORF_max_filt_A_uORF %>% filter(!(gene_id %in% NMD$gene_id))
length(unique(ORF_max_filt_A_uORF2$gene_id)) #1719 genes
length(intersect(ORF_max_filt_uORF_RS2$gene_id,ORF_max_filt_A_uORF2$gene_id)) #139
